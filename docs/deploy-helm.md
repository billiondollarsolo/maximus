...
| Tag style | Example |
| --- | --- |
| Semver | `ghcr.io/billiondollarsolo/maximus:0.1.4` |
...
```bash
# Verify
docker pull ghcr.io/billiondollarsolo/maximus:0.1.4
```
...
```bash
docker build -f docker/Dockerfile -t YOUR_REG/maximus:0.1.4 .
docker push YOUR_REG/maximus:0.1.4
# --set image.repository=YOUR_REG/maximus --set image.tag=0.1.4
```
