...
| Tag style | Example |
| --- | --- |
| Semver | `ghcr.io/billiondollarsolo/maximus:0.1.6-alpha` |
...
```bash
# Verify
docker pull ghcr.io/billiondollarsolo/maximus:0.1.6-alpha
```
...
```bash
docker build -f docker/Dockerfile -t YOUR_REG/maximus:0.1.6-alpha .
docker push YOUR_REG/maximus:0.1.6-alpha
# --set image.repository=YOUR_REG/maximus --set image.tag=0.1.6-alpha
```
