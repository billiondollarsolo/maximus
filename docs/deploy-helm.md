...
| Tag style | Example |
| --- | --- |
| Semver | `ghcr.io/billiondollarsolo/maximus:0.1.5` |
...
```bash
# Verify
docker pull ghcr.io/billiondollarsolo/maximus:0.1.5
```
...
```bash
docker build -f docker/Dockerfile -t YOUR_REG/maximus:0.1.5 .
docker push YOUR_REG/maximus:0.1.5
# --set image.repository=YOUR_REG/maximus --set image.tag=0.1.5
```
